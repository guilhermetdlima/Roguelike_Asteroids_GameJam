...
class BossShooterL(Enemy):
    ...
    def __init__(self, screen_width, screen_height, player, multiplier, bullet_config, sprite, bulletSprite, money=100, score=5000, speed=0, health=500, damage=5):
        ...
        #print("BALA SPRITE:", bulletSprite)
        x =  1 + deslocamento_x
        ...
        self.player = player
        self.bullet_config = BulletConfig(damage=1, speed=15, size=20)
    ...
    def update(self, enemy_bullets):
        ...
        if self.moving_down:
            ...
            limite_esquerda = 0  # Mais à direita
            limite_direita = 20   # Pouco à esquerda
        ...
        if current_time - self.last_shot_time >= self.shoot_cooldown:
            ...
            if self.health > self.max_health / 2:
                bullet = Bullet(x=cx, y=cy, angle=angle, config=self.bullet_config, spritesheet=self.bulletSprite)
                #print("SPRITESHEET:", self.bulletSprite)
    ...
    def draw(self, screen):
        rotated_sprite = pygame.transform.rotate(self.sprite, 270)
